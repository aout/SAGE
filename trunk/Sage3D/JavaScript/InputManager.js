...
InputManager = function(){
    ...
    this.root.viewPort.addEventListener('mousedown', this.mouseDown, false);
    this.root.viewPort.addEventListener("mouseup", this.mouseUp, false);
    this.root.viewPort.addEventListener("mousemove", this.mouseMove, false);
...
InputManager.prototype.mouseUp = function(event){
    ...
    input.registerMousePosition(event);
};
...
InputManager.prototype.registerMousePosition = function(event){
    var input = Root.getInstance().getModule('INPUT_MANAGER');
    input.mousePos.screenX = event.clientX + document.body.scrollLeft + document.documentElement.scrollLeft;
    input.mousePos.screenY = event.clientY + document.body.scrollTop + document.documentElement.scrollTop;
    ...
    input.mousePos.x = input.mousePos.screenX - viewportX;
    input.mousePos.y = input.mousePos.screenY - viewportY;
};
...
// BE CAREFUL BECAUSE IT'S RAISED BY A CALLBACK (SCOPE IS LOST)
